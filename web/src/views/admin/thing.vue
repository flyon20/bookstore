    ...
    <div class="page-view">
      <div class="table-operations">
        <a-space>
          ...
          <a-button type="primary" @click="handleBatchDelete" danger>批量删除</a-button>
...
//控制弹窗同步更新，标签选择器与分类选择器对应
const allTags = ref([]);
const handleClassificationChange = (selectedClassificationId) => {
  if (selectedClassificationId) {
    // 过滤标签数据，仅保留与选定分类ID相匹配的标签
    const filteredTags = allTags.value.filter(tag => tag.classificationId === selectedClassificationId);
    modal.tagData = filteredTags;
  } else {
    // 如果没有选择分类（或选择被清除），可以清空标签选择器或显示所有标签
    modal.tagData = [];
  }
};
onMounted(async () => {
...
//   })
// }
const getTagDataList = (classificationId) => {
  listTagApi({ classificationId: classificationId }).then(res => {
    console.log('Filtered tags:', res.data); // 调试日志，查看返回的标签数据
    modal.tagData = res.data;
    console.log('modal.tagData after update:', modal.tagData); // 这里打印modal.tagData的值
  }).catch(err => {
    console.error(err);
  });
};
...
const handleEdit = (record: any) => {
  ...
  // 重要：根据当前记录的classificationId过滤标签
  // 这里假设record.classificationId已经正确设置
  handleClassificationChange(record.classificationId);
};
