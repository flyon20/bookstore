<template>
  <div class="main-bar-view">
    ...
    <div class="home" @click="navigateToHome">
      <a-menu mode="horizontal" :selected-keys="currentRoute">
        <a-menu-item key="home" @click="navigateToHome">
          首页
        </a-menu-item>
      </a-menu>
    </div>
    <div class="content-list">
      <a-dropdown>
        <a class="ant-dropdown-link" @mouseover="dropdownVisible = true" @mouseleave="dropdownVisible = false">
          图书分类预览
        </a>
        <template #overlay>
          <a-menu :selected-keys="contentData.selectedKeys">
            <a-sub-menu v-for="item in contentData.cData" :key="item.key" :title="item.title">
...
const currentRoute = computed(() => {
  return route.name === 'portal' ? ['home'] : [];
});

const navigateToHome = () => {
  if (router.currentRoute.value.name !== 'portal') {
...
// onSelect方法，当选中菜单项时触发
// const onSelect = (selectedKeys) => {
//   contentData.selectedKeys = selectedKeys;
//   console.log('选中的项:', contentData.selectedKeys[0]);
...
//   // 根据params.c获取商品列表的逻辑
//   console.log('获取商品列表，分类ID:', params.c);
//   // 这里添加获取商品列表的API调用逻辑
// };
const initSider = () => {
...
a.ant-dropdown-link.ant-dropdown-trigger:hover{
  color: #1890ff;
}
.content-list{
  font-size: 18px;
